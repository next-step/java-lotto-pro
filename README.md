...
### Start from
- 클래스 메소드 구현으로 시작한후, 지속적인 리팩토링
- 객체지향 생활체조9가지 원칙과 클린코드 원칙을 통해 리팩토링.

### Extract Method
- Rule1 : Indent level = 1
- 함수는 한 가지 일만 잘 하기
- 함수당 추상화 수준은 하나로 만들기 (내려가기 규칙)
	- 우리가 함수를 만드는 이유는, 하나의 문제를, 더 낮은 레벨의 추상화 수준으로 나누어서 수행하기 위해서이다.
	- 함수가 한가지 일만 잘하려면, 모든 문장의 추상화 수준이 동일 해야 한다.
### Extract Class
- 모든 원시값과 문자열 포장하기
- 일급 컬렉션 쓰기
- 3개 이상의 인스턴스 변수를 가진 클래스를 쓰지 않기
- 메서드 인자 수를 최소화하기(0~2개)
	- Why?
		- 인자는 개념을 이해하기 어렵게 만들기 때문이다.
			- 상태를 갖지 않는 FP는?
		- 테스트 복잡도
			- 갖가지 인자 조합으로 함수를 검증해야 한다면? 
	- How?
		- 인자수 1
			- OK Case
				- Query Method
				- (Pure) Fuction Method(e.g, Map)
				- Command Method
			- NG Case
				- Flag인자! 함수가 한꺼번에 여러가지를 처리한다고 대놓고 공표하는 셈.
		- 인자수 2 이상
			- Constructor처럼 한 값을 표현하지도, new Point(0, 0)처럼 자연적인 순서가 있지도 않다면, 일부를 독자적인 클래스 변수로 선언하여, 개념을 표현해라.
