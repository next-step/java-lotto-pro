...
## 구현할 기능목록
* Model (O)
  * 수동으로 구매할 로또 개수를 입력받는 기능 (O)
    * 구입금액으로 가능한 최대의 구입개수 보다 같거나 작아야한다. (O)
    * 숫자형식 이어야한다. (O)
    * 입력받은 갯수만큼 로또를 발급하고 상태값을 관리해야한다. (O)
* View
  * 수동으로 구매할 로또 개수를 입력받는 UI (O)
  * 수동으로 구매할 로또 번호를 입력받는 UI (O)
  * 수동구매 + 자동구매의 조합으로 로또 번호를 출력하는 UI (O)
