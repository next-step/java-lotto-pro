...
- [X] 수동 로또번호 입력은 수동으로 구매한 로또 갯수만큼 반복한다.
- [X] 수동으로 입력하는 로또 번호는 1~45 사이 중복되지 않는 정수이다.  
- [X] 수동 구매 횟수 외 구매 갯수는 자동으로 구매된다.
- [X] 수동 구매 로또와 자동 구매 로또가 merge 되어 하나의 LottoTicket을 구성한다.
- [X] 구매된 정보 메세지를 수동과 자동 몇개씩인지 표현하도록 변경한다. 
