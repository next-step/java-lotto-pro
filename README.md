...
- [x] 예외처리
    - [x] 사용자가 입력한 모든 input 값에 대하여, 유효하지 않을 경우 재입력받는 기능 추가
    - [x] 수동 구매 개수가 0보다 작거나, 입력한 돈보다 많을 경우 예외처리
    - [x] 수동 구매 번호가 유효하지 않을 경우 예외처리(잘못된 입력부터 다시 입력받도록)
- [x] java8에 추가된 Optional을 적용해 NullPointerException이 발생하지 않도록 