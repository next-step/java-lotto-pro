...
- [ ] `로또번호`는 1 부터 45 까지의 임의의 수 1개로 구성되어 있다.
- [ ] `당첨번호`는 서로 다른 6개의 `로또번호`로 이루어져 있다.
- [ ] `로또복권`는 서로 다른 6개의 `로또번호`로 이루어져 있다.
- [ ] `구입금액` 을 입력하여 천원 단위당 하나의 `로또복권`를 생성할 수 있다.
- [ ] `로또복권`와 당첨번호와 비교하여 `당첨통계` 를 확인할 수 있다.
...
|한글명|영어명|뜻|
|:---|:---|:---
|로또복권| lottery | 로또 당첨번호를 예측한 서로 다른 6개의 로또번호 |
|로또번호| lottoNumber | 1 부터 45 까지의 임의의 숫자 |
|당첨번호| winningNumber | 맞춰야할 서로 다른 6개의 로또번호 |
