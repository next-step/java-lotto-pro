...
### 로또(자동)
#### 기능 요구사항
- [ ] 로또 구입 금액을 입력하면 구매한 로또 개수를 출력한다.
- [ ] 구입 금액이 음수이면 IllegalArgumentException 를 발생시킨다.
- [ ] 구입 금액이 로또 가격(1000)으로 나누어지지 않으면 IllegalArgumentException 를 발생시킨다.
- [ ] 로또 구매가 완료되면 6개의 로또 번호(1~45)를 랜덤으로 중복되지 않게 생성한다.
