...
class LottosTest {
    ...
    @Test
    void 수동으로구매한로또와_자동으로구매한로또를_보관한다() {
        List<LottoNumber> autoLottoNumber = Arrays.asList(new LottoNumber(Arrays.asList(1, 2, 3, 4, 5, 6)));
        List<LottoNumber> manualLottoNumber = Arrays.asList(new LottoNumber(Arrays.asList(1, 2, 3, 4, 5, 6)));

        Lottos lottos = new Lottos(autoLottoNumber, manualLottoNumber);
        assertThat(lottos.getLotto()).hasSize(2);
    }
    ...
    void 로또숫자의범위는_1에서_45여야한다() {
        ...
        Lottos lottos = new Lottos(Collections.emptyList(), new Seller().buyAuto(count, new RandomStrategy(arrangeNumber)));
    ...
    void 수동으로만_로또를_구매한다() {
        ...
        Lottos lottos = new Lottos(manualLotto,
                new Seller().buyAuto(0, new MockStrategy(Arrays.asList(1, 2, 3, 4, 5, 6)))
        );
    ...
    void 자동으로만_로또를_구매한다() {
        ...
        Lottos lottos = new Lottos(Collections.emptyList(),
                new Seller().buyAuto(autoCount, new MockStrategy(Arrays.asList(1, 2, 3, 4, 5, 6)
                )));
    ...
    void 수동_자동_을_섞어서_로또를_구매한다() {
        ...
        Lottos lottos = new Lottos(manualLotto,
                new Seller().buyAuto(3, new MockStrategy(Arrays.asList(1, 2, 3, 4, 5, 6))));
