...
class StringAddCalculatorTest {
    ...
    void splitAndSumOneNumber(String input, int expect) {
    ...
    void splitAndSumCommaOrColonDelimiter(String input, int expect) {
        int result = StringAddCalculator.splitAndSum(input);
        assertThat(result).isEqualTo(expect);
    }
    ...
    @DisplayName("음수를 포함한 문자열을 전달하면 IllegalArgumentException 예외가 발생한다.")
    @ParameterizedTest(name = "음수를 포함한 {0}를 전달하면 IllegalArgumentException 예외가 발생한다.")
    @ValueSource(strings = {"3,-1", "-1:-1", "1:2:-3"})
    void splitAndSumNegative(String input) {
        assertThatIllegalArgumentException()
                .isThrownBy(() -> StringAddCalculator.splitAndSum(input))
                .withMessage("음수가 입력되어 유효하지 않습니다.");
    }

    @DisplayName("숫자가 아닌 값이 포함된 문자열을 전달하면 IllegalArgumentException 예외가 발생한다.")
    @ParameterizedTest(name = "숫자가 아닌 값이 포함된 문자열 {0}를 전달하면 IllegalArgumentException 예외가 발생한다.")
    @ValueSource(strings = {"3,p", ":3", "1::3", "e:rr:or"})
    void splitAndSumNotNumber(String input) {
        assertThatIllegalArgumentException()
                .isThrownBy(() -> StringAddCalculator.splitAndSum(input))
