...
class LottoRuleTest {
	...
	void instance() {
		assertThatNoException()
			.isThrownBy(() -> LottoRule.of(Range.of(1, 45), 5));
	}

	@Test
	@DisplayName("범위 없이 객체화하면 IllegalArgumentException")
	void instance_nullRange_thrownIllegalArgumentException() {
		assertThatIllegalArgumentException()
			.isThrownBy(() -> LottoRule.of(null, 5))
	...
	void instance_lessThanOne_thrownIllegalArgumentException() {
		assertThatIllegalArgumentException()
			.isThrownBy(() -> LottoRule.of(Range.of(1, 45), 0))
	...
	void instance_gapLessThanCount_thrownIllegalArgumentException() {
		assertThatIllegalArgumentException()
			.isThrownBy(() -> LottoRule.of(Range.of(1, 45), Integer.MAX_VALUE))
			.withMessageMatching(
	...
	void differentCount(int target, boolean expected) {
		//when
		boolean different = LottoRule.of(Range.of(1, 45), 10)
	...
	void numbers() {
		//when
		int[] numbers = LottoRule.of(Range.of(1, 45), 10)
			.numbers();
	...
	void invalidNumber(int target, boolean expected) {
		//when
		boolean invalid = LottoRule.of(Range.of(1, 45), 10)
			.outOfRange(target);
