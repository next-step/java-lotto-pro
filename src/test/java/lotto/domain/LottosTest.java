...
class LottosTest {
    ...
    public void findWinnerTest() {
        Lotto lotto = Lotto.from(Arrays.stream(new int[]{1, 2, 3, 4, 5, 45}).boxed().collect(Collectors.toList()));
        Lotto winningNums = Lotto.from(Arrays.stream(new int[]{1, 2, 3, 4, 5, 6}).boxed().collect(Collectors.toList()));
