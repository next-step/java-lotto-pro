...
class LottoNumberPoolTest {
    ...
    private final LottoNumberPool lottoNumberPool = new LottoNumberPool();

    @DisplayName("랜덤 로또 생성시 6개의 LottoNumber를 가진 Lotto가 생성되는 것을 확인")
    @Test
    void generateLotto() {
        assertThat(lottoNumberPool.generateLotto().size()).isEqualTo(6);
    }

    @DisplayName("랜덤 로또 생성시 동일한 로또 번호가 중복으로 들어가지 않은 것을 확인")
    @RepeatedTest(10)
    void generateLottoNotDuplication() {
        Lotto lotto = lottoNumberPool.generateLotto();
