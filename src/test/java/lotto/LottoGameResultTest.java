...
import static lotto.rank.LottoRank.FIRST_PLACE;
import static lotto.rank.LottoRank.FOURTH_PLACE;
import static lotto.rank.LottoRank.NO_PRIZE;
import static lotto.rank.LottoRank.SECOND_PLACE;
import static lotto.rank.LottoRank.THIRD_PLACE;
...
public class LottoGameResultTest {
    ...
    private static Stream<Arguments> provideLottoGameResultWithYield() {
        ...
        List<LottoRank> testRankList2 = Arrays.asList(ranks);
        double testYield2 =
                (double) (THIRD_PLACE.getPrize() * 2 + FOURTH_PLACE.getPrize() * 3 + SECOND_PLACE.getPrize() * 1) / (
                        LottoGame.LOTTO_PRICE * 8);
        return Stream.of(
    ...
    private static Stream<List<LottoRank>> provideLottoRankList() {
        LottoRank[] ranks = {NO_PRIZE, THIRD_PLACE, THIRD_PLACE};
        List<LottoRank> testRankList1 = Arrays.asList(ranks);
        ranks = new LottoRank[]{NO_PRIZE, NO_PRIZE, NO_PRIZE, NO_PRIZE, THIRD_PLACE, THIRD_PLACE, FOURTH_PLACE,
