...
public class StatisticTest {
    ...
    void setUp() {
        ...
        fakePurchaseLottoNumbers.add(LottoNumbers.from(Arrays.asList(1, 2, 3, 4, 5, 45)));
        fakePurchaseLottoNumbers.add(LottoNumbers.from(Arrays.asList(1, 2, 3, 4, 44, 45)));
        fakePurchaseLottoNumbers.add(LottoNumbers.from(Arrays.asList(1, 2, 3, 43, 44, 45)));
    }

    @Test
    void 여섯개_맞춘_개수_가져오기() {
        Statistic statistic = new Statistic(LottoNumbers.from(Arrays.asList(1, 2, 3, 4, 5, 6)));
        statistic.countPrize(new PurchaseLottoNumbers(fakePurchaseLottoNumbers), LottoNumber.from(7));
    ...
    void 다섯개와_보너스_번호_맞춘_개수_가져오기() {
        Statistic statistic = new Statistic(LottoNumbers.from(Arrays.asList(1, 2, 3, 4, 5, 6)));
        statistic.countPrize(new PurchaseLottoNumbers(fakePurchaseLottoNumbers), LottoNumber.from(45));
