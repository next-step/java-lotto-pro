...
public class StatisticsTest {
    ...
    void 로또숫자비교_test() {
        ...
        Map<Rank, Integer> resultMap = statistics.culculate(winLotto, autoLottos, manualLottos);

        // then
        assertThat(resultMap.get(Rank.FIRST)).isEqualTo(0);
        ...
        assertThat(resultMap.get(Rank.MISS)).isEqualTo(13);
    }
    ...
    void 로또숫자비교2_test(Lotto lotto, Rank rank, int result) {
        // given
        autoLottos = Arrays.asList(lotto);
        ...
        // then
        assertThat(resultMap.get(rank)).isEqualTo(result);
    }

    @Test
    @DisplayName("자동 로또 총 구입 갯수는 14이다")
    void 로또_총갯수_test() {
        // when
        Statistics statistics = new Statistics();
        statistics.culculate(winLotto, autoLottos, manualLottos);
